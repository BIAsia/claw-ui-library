...
const sansFace = Archivo({
  subsets: ["latin"],
  variable: "--font-case-005-sans",
  weight: ["400", "500", "600", "700", "800"],
});

const serifFace = Cormorant_Garamond({
  subsets: ["latin"],
  variable: "--font-case-005-serif",
  weight: ["500", "600", "700"],
  style: ["normal", "italic"],
});
...
const navigation = ["Work ↗", "Studio", "Blog", "Contact"];

const featuredArticles: FeaturedArticle[] = [
  {
    number: "1",
    date: "12.06.2021",
    title: "Blonde – Widespread Acclaim",
    excerpt:
      "Blonde received widespread acclaim, with critics praising Ocean's introspective lyrics and the album's emotional precision.",
  },
  {
    number: "2",
    date: "12.06.2021",
    title: "Introspective Lyrics and Beats",
    excerpt:
      "When we toured Scotland we stopped at several self-sealing houses because hotels would not hold the same stillness.",
  },
];
...
function ScenicImage({ pointer, hovered }: { pointer: PointerState; hovered: boolean }) {
  return (
    <div className="relative aspect-[1.42/1] overflow-hidden border border-black/10 bg-[#d7d3cd]">
      ...
      <div className="absolute inset-x-0 bottom-[28%] h-[1px] bg-black/8" />
  ...
  return (
    ...
    >
      <div className="mb-3 flex items-start justify-between gap-4 text-[#161616]">
        <span className="text-[3rem] leading-none tracking-[-0.06em]">{article.number}</span>
        <span className="pt-2 text-[0.72rem] tracking-[-0.02em] text-[#161616]/70">{article.date}</span>
      </div>
...
export function Case005SelfEsteemEditorial() {
  ...
  return (
    ...
    >
      ...
      <style jsx>{`
        .case005-dot {
          animation: case005Pulse 4.6s cubic-bezier(0.45, 0, 0.25, 1) infinite;
        }

        @keyframes case005Pulse {
          0%,
          100% {
