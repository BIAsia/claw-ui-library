...
function CopyBlock({ x, y, size = 13, weight = 600, lines, align = "start", lineHeight = 1.2 }: CopyBlockProps) {
  return (
    <text
      x={x}
      y={y}
...
export function Case003JulieDoucet() {
  ...
  return (
    <main
      className={`${displayFace.variable} ${infoFace.variable} min-h-screen overflow-hidden bg-[#131313] text-white`}
    >
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1400px] items-center justify-center px-4 py-10 md:px-10 md:py-16">
        <div
          ref={containerRef}
          className="group relative w-full max-w-[860px] cursor-none"
          ...
          >
            ...
            >
              ...
              <g style={{ transform: `translate(${tilt.tx * -0.08}px, ${tilt.ty * -0.08}px)` }}>
                <CopyBlock x={304} y={41} lines={["Les femmes et la bande", "dessinée, un lien présent", "pourtant oublié.", "Redonnons-leur la visibilité", "qu’elles méritent"]} />
                <CopyBlock x={53} y={260} lines={["Exposition au Musée", "d’arts de Nantes", "en collaboration avec", "la Maison Fumetti"]} />
                <CopyBlock x={292} y={565} weight={500} lines={["Ici, les femmes bédéistes", "du 13 mai au 26 août 2023"]} />
                <text x="129" y="811" fill="#9b554c" fontFamily="var(--font-case-003-info)" fontSize="12" fontWeight="500" letterSpacing="-0.03em" transform="rotate(-89 129 811)">
