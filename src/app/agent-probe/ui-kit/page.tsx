...
import { uiKitProbe, uiKitProbeRoutes } from "@/lib/ui-kit-concept-probe";

export default function UiKitProbePage() {
  return (
    <main className="min-h-screen bg-[#070707] px-4 py-6 text-white md:px-8 md:py-8">
      <div className="mx-auto flex max-w-[1560px] flex-col gap-8">
        <header className="border-b border-white/10 pb-6">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em] md:text-5xl">{uiKitProbe.name}</h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-white/66 md:text-[15px]">{uiKitProbe.summary}</p>
        </header>

        <section className="grid gap-4 xl:grid-cols-3">
          {uiKitProbeRoutes.map((item) => (
