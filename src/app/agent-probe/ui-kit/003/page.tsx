...
const displayFace = Barlow_Condensed({
  subsets: ["latin"],
  variable: "--font-ui-kit-003-display",
  weight: ["400", "500", "600", "700"],
});

const infoFace = IBM_Plex_Sans({
  subsets: ["latin"],
  variable: "--font-ui-kit-003-info",
  weight: ["400", "500", "600", "700"],
});
...
function CopyBlock({ x, y, size = 13, weight = 600, lines, align = "start", lineHeight = 1.2, fill = note }: CopyBlockProps) {
  return (
    <text
      ...
      fill={fill}
      textAnchor={align}
      fontFamily="var(--font-ui-kit-003-info)"
      fontSize={size}
      fontWeight={weight}
...
export default function UiKit003Page() {
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1400px] items-center justify-center px-4 py-10 md:px-10 md:py-16">
        <div ref={containerRef} className="group relative w-full max-w-[860px] cursor-none" style={{ perspective: 1800 }}>
          ...
          <div
            ...
            style={{ transform: `rotateX(${tilt.rx}deg) rotateY(${tilt.ry}deg) translate3d(${tilt.tx}px, ${tilt.ty}px, 0)` }}
          >
            ...
            <svg
              ...
              role="img"
              aria-label="UI Kit poster surface with oversized condensed title fragments, sparse component notes, pink collage shapes, and pointer-reactive hand-drawn marks."
              style={{ fontFamily: "var(--font-ui-kit-003-display)" }}
            >
              <rect width="736" height="1051" fill={paper} />

              <g style={{ transform: `translate(${tilt.tx * -0.12}px, ${tilt.ty * -0.12}px)` }}>
                <LargeText x={52} y={101} size={82} lines={["UI", "KIT"]} />
                <LargeText x={446} y={296} size={82} lines={["ENTRY", "POSTER"]} />
                <LargeText x={54} y={430} size={80} lines={["TOKENS", "PARTS"]} />
                <LargeText x={446} y={603} size={80} lines={["MODULE", "DRIFT"]} />
                <LargeText x={53} y={718} size={84} lines={["SYSTEM", "LAUNCH"]} />
                <LargeText x={283} y={894} size={84} lines={["SHIP", "READY"]} />
              </g>

              <g style={{ transform: `translate(${tilt.tx * -0.08}px, ${tilt.ty * -0.08}px)` }}>
                <CopyBlock x={304} y={41} lines={["campaign layer for the kit", "show the mood before the library opens"]} />
                <CopyBlock x={53} y={260} lines={["probe 003 / ui kit", "entry surface for parts and tokens", "before browsing, before docs"]} />
                <CopyBlock x={292} y={565} weight={500} lines={["starter blocks for shipping interfaces", "type / motion / layout / release"]} />
                <text
                  x="129"
                  y="811"
                  fill={note}
                ...
                >
                  <tspan x="129" dy="0">small index markers, sparse tags,</tspan>
                  <tspan x="129" dy="1.18em">and one authored poster object.</tspan>
                  <tspan x="129" dy="1.18em">this page sells the system mood</tspan>
                  <tspan x="129" dy="1.18em">without becoming the system browser.</tspan>
                </text>
              </g>
