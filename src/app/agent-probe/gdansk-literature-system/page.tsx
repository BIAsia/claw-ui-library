...
import { case027StyleFamily, case027StyleFamilyRoutes } from "@/lib/case-027-style-family";

export default function GdanskLiteratureSystemProbePage() {
  return (
    <main className="min-h-screen bg-[#ece6da] px-4 py-6 text-[#1d1914] md:px-8 md:py-8">
      <div className="mx-auto flex max-w-[1560px] flex-col gap-8">
        <header className="border-b border-black/10 pb-6">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em] md:text-5xl">{case027StyleFamily.name}</h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-black/66 md:text-[15px]">{case027StyleFamily.summary}</p>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-black/76">
            <span className="font-medium text-black/92">Mode lock:</span> {case027StyleFamily.positioning}
          </p>
        </header>
        ...
        <section className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
          <article className="border border-black/10 bg-white/60 p-5">
            ...
            <ul className="mt-4 space-y-3 text-sm leading-6 text-black/78">
              {case027StyleFamily.fixedConstraints.map((item) => (
                <li key={item}>• {item}</li>
              ))}
            </ul>
          </article>
          <article className="border border-black/10 bg-white/60 p-5">
            ...
            <ul className="mt-4 space-y-3 text-sm leading-6 text-black/78">
              {case027StyleFamily.desiredSignals.map((item) => (
                <li key={item}>• {item}</li>
              ))}
            </ul>
          </article>
        ...
        <section className="grid gap-4 xl:grid-cols-2">
          {case027StyleFamilyRoutes.map((item) => (
            <article key={item.route} className="overflow-hidden border border-black/10 bg-white/60">
              ...
              <iframe title={item.title} src={item.route} className="h-[940px] w-full bg-[#ebe5da]" />
            </article>
          ))}
        </section>
      </div>
