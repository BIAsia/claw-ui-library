...
import { case025StyleFamily, case025StyleFamilyRoutes } from "@/lib/case-025-style-family";

export default function DadFestivalTabsProbePage() {
  return (
    <main className="min-h-screen bg-[#09090d] px-4 py-6 text-white md:px-8 md:py-8">
      <div className="mx-auto flex max-w-[1560px] flex-col gap-8">
        <header className="border-b border-white/10 pb-6">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em] md:text-5xl">{case025StyleFamily.name}</h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-white/66 md:text-[15px]">{case025StyleFamily.summary}</p>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-white/76">
            <span className="font-medium text-white/92">Mode lock:</span> {case025StyleFamily.positioning}
          </p>
        </header>
        ...
        <section className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
          <article className="border border-white/10 bg-white/[0.02] p-5">
            ...
            <ul className="mt-4 space-y-3 text-sm leading-6 text-white/78">
              {case025StyleFamily.fixedConstraints.map((item) => (
                <li key={item}>• {item}</li>
              ))}
            </ul>
          </article>
          <article className="border border-white/10 bg-white/[0.02] p-5">
            ...
            <ul className="mt-4 space-y-3 text-sm leading-6 text-white/78">
              {case025StyleFamily.desiredSignals.map((item) => (
                <li key={item}>• {item}</li>
              ))}
            </ul>
          </article>
        ...
        <section className="grid gap-4 xl:grid-cols-2">
          {case025StyleFamilyRoutes.map((item) => (
            <article key={item.route} className="overflow-hidden border border-white/10 bg-white/[0.02]">
              ...
              <iframe title={item.title} src={item.route} className="h-[920px] w-full bg-[#151521]" />
            </article>
          ))}
        </section>
      </div>
