...
export default function HouseholdTreasuryPage() {
  const monthly = useCountUp(412, 1600, 2);
  const remaining = useCountUp(9420, 2100, 0);
  const reserve = useCountUp(18440, 1900, 0);
  const goalProgress = useMemo(() => Math.min((reserve / 22000) * 100, 100), [reserve]);
  const sliderProgress = useCountUp(71, 1600, 1);
        ...
        <div className="grid w-full max-w-[900px] gap-4 lg:grid-cols-[1fr_1.1fr_1.05fr]">
          <div className="grid gap-4">
            <Panel className="bg-[#040404] text-white">
              <h1 className="text-[clamp(2.8rem,7vw,4.3rem)] leading-[0.86] font-[600] tracking-[-0.09em]">Thursday</h1>
              <p className="mt-1 text-[1.7rem] tracking-[-0.06em] text-white/35">March 19th</p>
            </Panel>
            ...
            <Panel className="bg-[#c4b6df] text-[#101010]">
              <p className="max-w-[9ch] text-[clamp(1.7rem,3.6vw,2.3rem)] leading-[0.95] font-[600] tracking-[-0.07em]">
                Shared money stays calm when every deadline, limit, and reserve is visible in one place.
              </p>
              <p className="mt-3 max-w-[11ch] text-[1.8rem] leading-[0.95] font-[600] tracking-[-0.07em]">Household Treasury keeps the whole month legible.</p>
            </Panel>
            ...
            <Panel className="bg-[#050505] text-white">
              ...
              <div className="mt-3 grid grid-cols-7 gap-y-2 text-center text-[1.8rem] tracking-[-0.05em]">
                {days.map((day) => (
                  <span
                    key={day}
                    className={`mx-auto flex h-9 w-9 items-center justify-center rounded-full ${day === 22 ? "bg-[#ff5a2b] text-black" : "text-white"}`}
                  >
                    {day}
                  </span>
                ))}
            ...
            <Panel className="bg-[#a89f00] text-[#111]">
              <div className="inline-flex rounded-full border border-[#111] px-4 py-1.5 text-[1rem]">Buffer Health</div>
              <p className="mt-5 max-w-[18ch] text-[1rem] leading-[1.1]">Reserve strength across the next 12 weeks, calculated from bills, income timing, and expected family spend.</p>
              <div className="mt-8 grid h-40 grid-cols-12 items-end gap-2">
                {Array.from({ length: 12 }, (_, index) => 28 + ((index * 19) % 70)).map((height, index) => (
          ...
          <div className="grid gap-4">
            <Panel className="bg-[#f8ea2c] text-[#111]">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[0.9rem] uppercase tracking-[0.14em] text-black/55">Weekly Drift</p>
                  <p className="mt-1 text-[2.1rem] leading-none font-[700] tracking-[-0.08em]">{formatCurrency(monthly, 2)}</p>
                </div>
                <div className="text-right">
                  <p className="text-[0.9rem] uppercase tracking-[0.14em] text-black/55">Month Remaining</p>
            ...
            <Panel className="bg-[#eef2e7] text-[#101010]">
              ...
              <div className="mt-5 border-t border-black/10 pt-4">
                <p className="text-[0.9rem] uppercase tracking-[0.12em] text-black/55">Envelope Limits</p>
                <div className="mt-4 space-y-3">
          ...
          <div className="grid gap-4">
            <Panel className="bg-[#050505] text-white">
              ...
              <div className="mt-4 space-y-3">
                <div className="flex items-center justify-between rounded-full bg-[#eef2e7] px-5 py-3 text-[#111]">
                  ...
                  <span className="text-[2rem] leading-none tracking-[-0.08em]">MOVE TO RESERVE</span>
                </div>
                <div className="flex items-center justify-between rounded-full bg-[#ff5a2b] px-5 py-3 text-[#111]">
                  <span className="text-[1.8rem]">◎</span>
                  <span className="text-[2rem] leading-none tracking-[-0.08em]">SPLIT HOUSE BILLS</span>
                </div>
              </div>
            </Panel>

            <Panel className="bg-[#f8ea2c] text-[#111]">
              <div className="inline-flex rounded-full border border-[#111] px-4 py-1.5 text-[1rem]">Readiness</div>
              <p className="mt-5 text-[clamp(3rem,8vw,5rem)] leading-none font-[700] tracking-[-0.1em]">
                <AnimatedPercent value={12.68} />
              </p>
              <p className="mt-2 text-[1rem]">Compared to last month</p>
              <div className="mt-6 grid grid-cols-[1.1fr_0.8fr] gap-3">
                <div className="h-28 bg-black" />
                <div className="flex flex-col justify-between">
                  <div className="h-10 bg-black/22" />
                  <p className="text-[0.78rem] leading-[1.1] text-black/55">Projected coverage +9.42%</p>
                </div>
              </div>
            ...
            <Panel className="bg-[#ff5a2b] text-[#111]">
              ...
              <div className="mt-5 space-y-3 text-[2rem] leading-none tracking-[-0.08em]">
                {["Transfers", "Upcoming Bills", "Allowances", "Repairs", "Subscriptions", "Help"].map((item) => (
            ...
            <Panel className="flex items-center justify-center bg-[#bfb1db] text-[#111]">
              <p className="text-[2.35rem] leading-none tracking-[-0.08em]">i HOUSEHOLD.TREASURY</p>
            </Panel>
          </div>
        </div>
      </section>
